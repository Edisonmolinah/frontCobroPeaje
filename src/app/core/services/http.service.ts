...
export class HttpService {
  ...
  public doPostP<T, R>(serviceUrl: string, body: T, opts?: Options): Observable<R> {
    const ropts = this.createOptions(opts);
  ...
  public doPutP<T, R>(serviceUrl: string, body: T, opts?: Options): Observable<R> {
    const ropts = this.createOptions(opts);

    return this.http.put<R>(serviceUrl, body, ropts);
  }
  public doPut<T>(serviceUrl: string, body: T, opts?: Options): Observable<T> {
    const ropts = this.createOptions(opts);

    return this.http.put<T>(serviceUrl, body, ropts);
  }
